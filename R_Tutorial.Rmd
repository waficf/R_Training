...
```{r}
train_data <- read.csv("train.csv", na.strings=c("","NA"), as.is=TRUE)
test_data <- read.csv("test.csv", na.strings=c("","NA"), as.is=TRUE)
...
```{r}
summary(full$Age)
summary(full[c('Age', 'Fare')])
```

> There is a huge difference between the min and Q1 and Q3 and max which 
indicates that there is a spread of data for people age above 39 and below 21.
On the other hand we can see that between Q1 and Q3 there is a small gap showing
a more tight group around the center.

```{r}
boxplot(full$Age, ylab='Age', main='Box Plot of Age')
```

```{r}
hist(full$Fare, main='Fare Histogram', ylab='Fare')
```


```{r}
hist(full$Age, main='Age Histogram', ylab='Age', breaks=10)
```

```{r}
model_tbl <- table(full$Sex)
prop.table(model_tbl)
```

```{r}
plot(x = full$Age, y = full$Fare, xlab = 'Age', ylab='Fare')
```

```{r}
#CrossTable(x = full$Sex, y = full$Cabin)
```

> This is a machine learning classification algorithm to detect if a tumor is
> malignant or bening us K Nearest Neighbor
...
# CHECK IF YOU CAN APPLY THE ABOVE TO THE AMAZON DATA AS YOU NOW UNDERSTAND THE VECTOR AND THE LIST AND THE DATAFRAME IN R
