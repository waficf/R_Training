...
#### 5.5.1 SVM
```{r}
svm_classifier <- ksvm(Survived ~ ., data = norm_train, kernel = "vanilladot") #kernelrbf: rbfdot

svm_predictor <- predict(svm_classifier, norm_test)

CrossTable(x=norm_test_labels, y=svm_predictor,
           prop.chisq = FALSE, prop.t = FALSE,
           dnn = c('actual', 'predicted'))
```
#### 5.5.1 SVM (Auto Parameter Tuning)
```{r}
#modelLookup("svmRadial")
ctrl <- trainControl(method = "repeatedcv", number = 10, repeats = 5)
grid_svm <- expand.grid(.sigma= c(0, 5, 10, 15), 
                        .C= c(0:5))

set.seed(300)
m_svm <- train(Survived ~ ., 
              data = full_data, 
              method = "svmRadial",
              metric = 'Kappa', 
              trControl = ctrl, 
