...
str(full_data)
```
We have 12 variables with no nulls that can be used in prediction models

## 5. Predictive Models

#### 5.1 Evaluating variable importance
```{r}
...
```
We can see dummy variable Male in Sex and Mr in Title most important probably 
because they make the majority of the data available
...
########################
APPLY ALL CLASSIFICATION MODELS IN THE BOOK that can be applied here
########################
...
#### 5.1 Splitting data into train and test datasets
```{r}
split = 0.7
trainIndex <- createDataPartition(full_data$Survived, p=split, list=FALSE)
train <- full_data[trainIndex,]
test <- full_data[-trainIndex,]

train_labels <- full_data[trainIndex,]$Survived
test_labels <- full_data[-trainIndex,]$Survived
```
...
titanic_rpart <- rpart(Age ~ Pclass + Sex + SibSp + Parch + Fare + Embarked + Title + F_Size_D, data=data_tit, method="anova")
```
...
rf <- randomForest(Survived ~ Pclass + Sex + Age + SibSp + Parch + 
                     Fare + Embarked + Title + Child + Mother, data = train)
